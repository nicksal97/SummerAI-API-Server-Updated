...
def using_box_find_center_point(boxes,postion_ls, output_directory, input_filename):
    center_points = []
    ...
    for input_filename in os.listdir(input_directory):
        if input_filename.endswith(".json"):
            ...
            for line,polygone_area in zip(data['actual_coordinates'],data['polygone_area']):
                calculate_center_points_geojson(line,polygone_area, output_filename)
...
def index(request):
    ...
    if request.method == "POST":
        ...
        folder_path = 'pra/Railways_QGSI_Dataset'
        count = 0
        ...
        #     base_name, extension = os.path.splitext(file.name)
            ...
            if extension == ".jgw":
                ...
                flag, ori_path, boxes, img_count, box_ls, postion_ls, center_ls = prediction(image)
                output_directory = 'static/centers'
                using_box_find_center_point(boxes,postion_ls, output_directory, file_name)
                output_path = f'{output_folder}/{file_name}'
                if flag == True:
                    # image = Image.open(output_path)
                    width, height = image.size
